...
const slugify = (nome: string): string =>
  nome
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
const presentesComSlug = presentes.map(presente => ({
  ...presente,
  slug: slugify(presente.nome),
}));
...
const PresentesPage = () => {
  ...
  const totalPaginas = Math.ceil(presentesComSlug.length / itensPorPagina);

  const presentesVisiveis = presentesComSlug.slice(
  ...
  return (
    <div className="pagina-presentes">
      ...
      <p className="intro">
        Abaixo estão alguns presentes simbólicos para nos ajudar a montar nosso
        lar. Obrigado por fazer parte disso!
      </p>
      ...
      <div className="presentes-grid">
        {presentesVisiveis.map((presente, index) => (
          <div key={index} className="presente-card">
            ...
            <button
              onClick={() => navigate(`/presente/${presente.slug}`)}
            >
              Presentear
            </button>
          </div>
